...
_ = my_utils.test(test_dataloader, model, loss_fn, device, False )
my_utils.print_acc(test_data, test_dataloader, model, device, False)
...
_ = my_utils.test(test_dataloader, nemo_model, loss_fn, device, False)
my_utils.print_acc(test_data, test_dataloader, nemo_model, device, False)
...
nemo_model.id_stage()
#print(nemo_model)
_ = my_utils.test(test_dataloader, nemo_model, loss_fn, device, integer=True)
my_utils.print_acc(test_data, test_dataloader, nemo_model, device, integer = True)
...
my_utils.write_act_parameters(nemo_model, act_json)

#write file with all intermediate results
in_t = (torch.randint(low=0, high=256, size=(1,1,28,28), dtype=torch.float32)).to(device)
my_utils.write_intermidiate_results(nemo_model, in_t, ex_dir)
my_utils.write_weights(nemo_model,ex_dir)
