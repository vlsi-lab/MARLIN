...
This file was downloaded and modified in order to support 9x9 multiplication.
*/
...
#include STR(axx_mults_9x9/EXPAND(AXX_MULT).h)
 ...
 out_t *C, int ldc)
{
    ...
    for(int i = 0; i < M; ++i) {
        for(int j = 0; j < N; ++j) {
            ...
            //} 
            for(int k = 0; k < b; k+=8) {
                //if (A[i*lda+k]>511 || A[i*lda+k+1]>511 || A[i*lda+k+2]>511 || A[i*lda+k+3]>511|| A[i*lda+k+4]>511|| A[i*lda+k+5]>511|| A[i*lda+k+6]>511 || A[i*lda+k+7]>511)
                //  printf("A> 511\n");
                if (A[i*lda+k]<0 || A[i*lda+k+1]<0 || A[i*lda+k+2]<0 || A[i*lda+k+3]<0 || A[i*lda+k+4]<0 || A[i*lda+k+5]<0|| A[i*lda+k+6]<0 || A[i*lda+k+7]<0){
                  printf("A< 0\n");
                  printf("A[0]=%d, A[1]= %d, A[2] = %d, A[3]=%d, A[4]=%d, A[5]=%d, A[6]=%d, A[7]=%d\n", A[i*lda+k],A[i*lda+k+1],A[i*lda+k+2],A[i*lda+k+3],A[i*lda+k+4],A[i*lda+k+5],A[i*lda+k+6],A[i*lda+k+7]);
                  printf("A[0]=%d, A[1]= %d, A[2] = %d, A[3]=%d, A[4]=%d, A[5]=%d, A[6]=%d, A[7]=%d\n",(uint16_t)A[i*lda+k],(uint16_t)A[i*lda+k+1],(uint16_t)A[i*lda+k+2],(uint16_t)A[i*lda+k+3],(uint16_t)A[i*lda+k+4],(uint16_t)A[i*lda+k+5],(uint16_t)A[i*lda+k+6],(uint16_t)A[i*lda+k+7]);
                }
                //if (B[j*K+k]>511 || B[j*K+k+1]>511 || B[j*K+k+2]>511 || B[j*K+k+3]>511|| B[j*K+k+4]>511|| B[j*K+k+5]>511|| B[j*K+k+6]>511 || B[j*K+k+7]>511)
                //  printf("B> 511\n");
                if (B[j*K+k]<0 || B[j*K+k+1]<0 || B[j*K+k+2]<0 || B[j*K+k+3]<0 || B[j*K+k+4]<0 || B[j*K+k+5]<0 || B[j*K+k+6]<0 || B[j*K+k+7]<0)
                  printf("B< 0\n");
                temp_arr[0] += lut[(uint16_t)A[i*lda+k]][(uint16_t)B[j*K+k]];
                temp_arr[1] += lut[(uint16_t)A[i*lda+k+1]][(uint16_t)B[j*K+k+1]];
                temp_arr[2] += lut[(uint16_t)A[i*lda+k+2]][(uint16_t)B[j*K+k+2]];
                temp_arr[3] += lut[(uint16_t)A[i*lda+k+3]][(uint16_t)B[j*K+k+3]];
                temp_arr[4] += lut[(uint16_t)A[i*lda+k+4]][(uint16_t)B[j*K+k+4]];
                temp_arr[5] += lut[(uint16_t)A[i*lda+k+5]][(uint16_t)B[j*K+k+5]];
                temp_arr[6] += lut[(uint16_t)A[i*lda+k+6]][(uint16_t)B[j*K+k+6]];
                temp_arr[7] += lut[(uint16_t)A[i*lda+k+7]][(uint16_t)B[j*K+k+7]];
            }
            
            temp += temp_arr[0]+temp_arr[1]+temp_arr[2]+temp_arr[3]+temp_arr[4]+temp_arr[5]+temp_arr[6]+temp_arr[7];
            for(int k = b; k < K; k++) {
                temp += lut[(uint16_t)A[i*lda+k]][(uint16_t)B[j*K+k]];
            } 
...
static void slow_conv2d_update_output_frame(
  ...
  if (bias.defined()) {
   ...
   my_addmm <int16_t, int> (output2d, weight, finput, 0, 1); //changed input type for my_addmm
   //   });
